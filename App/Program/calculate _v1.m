...
cd(CAL_PASS)
clearvars chr col GET_PASS i j List row str CAL_PASS
%% 関数の設定
clear coordinates3
clear coordinates1
clear diagonal
...
difpa = matrix2vectorD(pa_1,pa_2,pa_3,2,'HEAD');
...
%% rhomin - M*umin < 0
fprintf('\n\n\n-----------------rho-3----------------\nrhomin - M*umin < 0\n------------------------------------\n')
[B, NEXTCOL] = diagonal(B,NEXTCOL,rhomin_top, rhomin_size,1,'rhomin',0);
[B, NEXTCOL] = diagonal(B,NEXTCOL,umin_top  , umin_size  ,-M,'-M*umin'  ,1);
%% rhomax - M*umax < 0
fprintf('\n\n\n-----------------rho-4-----------------\nrhomax - M*umax < 0\n------------------------------------\n')
[B, NEXTCOL] = diagonal(B,NEXTCOL,rhomax_top, rhomax_size,1,'rhomax',0);
[B, NEXTCOL] = diagonal(B,NEXTCOL,umax_top  , umax_size  ,-M,'-M*umin'  ,1);

%% B(theta_u - theta_v) < fmax
fprintf('\n\n\n-----------------rho-5-----------------\nB(theta_u - theta_v) < fmax\n------------------------------------\n')
b = barray(b,NEXTCOL,fmax,'fmax');
[B,NEXTCOL]  = substitution(B,NEXTCOL,theta_top,Bs,t,'B(theta_u-theta_v)',1);
%% -B(theta_u - theta_v) < fmax
fprintf('\n\n\n-----------------rho-6-----------------\n-B(theta_u - theta_v) < fmax\n------------------------------------\n')
b = barray(b,NEXTCOL,fmax,'fmax');
[B,NEXTCOL]  = substitution(B,NEXTCOL,theta_top,-Bs,t,'-B(theta_u-theta_v)',1);
%% B(theta_u - theta_v) + M*umin < M-fmax
fprintf('\n\n\n-----------------rho-7-----------------\nB(theta_u - theta_v) + M*umin < fmax\n------------------------------------\n')
b = barray(b,NEXTCOL,M-fmax,'M-fmax');
[B,NEXTCOL]  = substitution(B,NEXTCOL,theta_top,Bs,t,'B(theta_u-theta_v)',0);
...
fprintf('\n\n\n-----------------lamA-2-----------------\n-(lambdaA_u - lambdaA_v) < 0\n------------------------------------\n')
[B, NEXTCOL] = doubleDiagonal(B,NEXTCOL,lambdaA_top,lambdaA_size,1,-1,'-lambdaA',1,t);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% -PDR < 0
fprintf('\n\n\n-----------------pdr-1-----------------\n-PDR < 0\n------------------------------------\n')
...
[B, ~] = diagonal(B,NEXTCOL,Ppv_top,Ppv_size, 1,'P(Bal+)',1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 等式制約
fprintf('↓↓↓等式制約↓↓↓\n')
...
[Aeq,NEXTCOL]  = substitution(Aeq,NEXTCOL,gamma_top,1,t,'gamma',0);
%% 目的関数
fprintf('↓↓↓目的関数↓↓↓\n')
...
f = farray(f,rhomin_top,-fmax,'rhomax');
f = farray(f,phiAmax_top,difpa,'phiAmax');
